  ...
  methods: {
    fetchData(id) {
      fetchArticle(id).then(response => {
        ...
        this.postForm.content_short = response.data.summary
    ...
    submitForm() {
      this.$refs.postForm.validate(valid => {
        if (valid) {
          this.loading = true
          createArticle(this.postForm.title, this.postForm.content_short, this.postForm.content, this.postForm.author.id, 1, this.postForm.display_time).then(response => {
            this.$notify({
              title: '成功',
              message: '发布文章成功',
              type: 'success',
